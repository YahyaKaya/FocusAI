...
import { useSessionNotification, cancelSessionNotification } from "../../lib/useSessionNotification";
...
export default function SessionScreen() {
  ...
  // Refs
  const startTimestampRef = useRef<number>(activeSessionStart ?? Date.now());
  const pausedElapsedRef = useRef<number>(0);
  const breakStartRef = useRef<number | null>(null);
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const appStateRef = useRef(AppState.currentState);
  const isEndingRef = useRef(false);
  const statusRef = useRef<"ACTIVE" | "PAUSED">("ACTIVE");

  useSessionNotification(sessionType, plannedDuration, elapsed, status === 'ACTIVE', activeSessionStart ?? undefined);

  // Keep statusRef in sync so interval closure can read it without stale closure
  useEffect(() => {
    statusRef.current = status;
  }, [status]);

  // Main timer — single source of truth
  useEffect(() => {
    const sessionStart = activeSessionStart ?? Date.now();
    startTimestampRef.current = sessionStart;
    ...
    // Set initial elapsed from real session start
    setElapsed(Math.floor((Date.now() - sessionStart) / 1000));

    // Start interval
    intervalRef.current = setInterval(() => {
      if (statusRef.current === "PAUSED") return;
      setElapsed(pausedElapsedRef.current + Math.floor((Date.now() - startTimestampRef.current) / 1000));
    }, 1000);

    // AppState handler — recalculate when coming back to foreground
  ...
  const ss = useMemo(() => StyleSheet.create({
    ...
    headerTitle: { fontSize: 16, fontWeight: "600", color: colors.onSurfaceVariant },
    headerTimer: { fontSize: 18, fontWeight: "800", color: colors.primary, letterSpacing: -0.5 },
    canvas: {
      alignItems: "center",
    ...
    pillText: { fontSize: 14, fontWeight: "600", color: colors.onTertiaryContainer, letterSpacing: 0.4 },
    timerCircle: {
      width: 288,
      height: 288,
      borderRadius: 144,
    ...
    timerDigits: { fontSize: 72, fontWeight: "700", color: colors.onSurface, letterSpacing: -3 },
    timerSub: { fontSize: 11, fontWeight: "500", color: colors.onSurfaceVariant, opacity: 0.6, marginTop: 8, letterSpacing: 3 },
    stats: { alignItems: "center", marginBottom: 32 },
    statPrimary: { fontSize: 14, fontWeight: "500", color: colors.onSurfaceVariant, letterSpacing: 0.3 },
    progressRow: {
      flexDirection: "row",
      alignItems: "center",
      gap: 10,
    ...
    progressLabel: { fontSize: 12, color: colors.onSurfaceVariant, fontWeight: "600", minWidth: 36, textAlign: "right" },
    typeRow: {
      flexDirection: "row",
      alignItems: "center",
      justifyContent: "space-between",
    ...
    btnSecondary: { height: 56, backgroundColor: colors.surfaceContainerHigh, borderRadius: 12, alignItems: "center", justifyContent: "center" },
    btnSecondaryText: { fontSize: 16, fontWeight: "600", color: colors.onSurface },
    btnPrimary: { height: 56, backgroundColor: colors.primary, borderRadius: 12, alignItems: "center", justifyContent: "center" },
    ...
    modalOption: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: colors.surfaceContainer },
  ...
  return (
    <SafeAreaView style={ss.root}>
      ...
      <ScrollView
        style={{ flex: 1 }}
        contentContainerStyle={ss.canvas}
